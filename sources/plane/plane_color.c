...
/*   Updated: 2023/03/23 00:48:29 by gamoreno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((((int)v_abs(integer_part(new_inter.x * mrt->plane[index].chess_ctrl))
				% 2 == 1 && (int)v_abs(integer_part(new_inter.y
						* mrt->plane[index].chess_ctrl)) % 2 == 1))
		|| ((int)v_abs(integer_part(new_inter.x * mrt->plane[index].chess_ctrl))
			% 2 == 0 && (int)v_abs(integer_part(new_inter.y
					* mrt->plane[index].chess_ctrl)) % 2 == 0))
		return (mrt->plane[index].color1);
	return (color);
}
...
t_rgb	pln_col_case2(t_mrt *mrt, int index, t_vec new_inter, t_rgb color)
{
	...
	c = 2;
	if (mrt->plane[index].chess_ctrl % 2 == 0)
		c = 1;
	if (((int)v_abs(integer_part((new_inter.x - 1)
					* mrt->plane[index].chess_ctrl) + c) % 2 == 1
			&& (int)integer_part(new_inter.y * mrt->plane[index].chess_ctrl)
			% 2 == 1) || ((int)v_abs(integer_part((new_inter.x - 1)
					* mrt->plane[index].chess_ctrl) + c) % 2 == 0
